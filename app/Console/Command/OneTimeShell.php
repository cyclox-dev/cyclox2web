...
class OneTimeShell extends AppShell
{
	...
	/**
	 * 2017-18 シーズンのリザルトから降格処理を行なう
	 * > cd app ディレクトリ
	 * > Console/cake one_time execCategoryDown1718 0 100
	 * > Console/cake one_time execCategoryDown1718 0 100 logonly
	 * logonly オプションでは Transaction をコミットせず、ログのみ出力する。
	 */
	public function execCategoryDown1718()
	{
		if (!isset($this->args[0]) || !isset($this->args[1])) {
			$this->out('2つの引数（整数／offset 位置, 処理件数）が必要です。');
			return;
		...
		$opt = array(
			...
			'conditions' => array(
				...
				// offset を維持するため、cancel_date => null などは手動カウントする。引数で offset, size を指定するため、
				'OR' => array(
		...
		$this->log('Log of execCategoryDown1718 start ---', LOG_DEBUG);
		...
		try {
			...
			foreach ($crs as $cr) {
				...
				if (!empty($cr['CategoryRacer']['apply_date']) && $cr['CategoryRacer']['apply_date'] > '2018-03-31') {
					++$skipNewApplied;
					continue; // offset 値をキープしたいので、find の条件文には入れていない
				}
				...
				$query .= ' where entry_racers.deleted = 0'
						...
						. " and meets.deleted = 0"
						. " and meets.at_date > '2017-08-01' and meets.at_date < '2018-04-01'"
				...
				if ($pt < 3) {
					...
					$categoryRacer = array(
						...
						'category_code' => $catTo,
						'apply_date' => '2018-04-01',
						'reason_id'=> CategoryReason::$SEASON_DOWN->ID(),
						'reason_note' => '2017-18シーズン成績の降格処理による'
					);
			...
			$this->TransactionManager->rollback($transaction);
			$this->log('以下のエラーにより異常終了しました。' . $e->getMessage(), LOG_ERR);
		}
		$this->log('Log of execCategoryDown1718 end ---', LOG_DEBUG);
	}
